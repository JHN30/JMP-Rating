DROP TABLE IF EXISTS silver.eurocup_player_jei_combined;

WITH filtered AS (
    SELECT *
    FROM silver.eurocup_players
    WHERE CAST(SUBSTRING(season_code, 2, 4) AS INT) <= 2025
),
player_jei AS (
    SELECT
        player_id,
...
weighted AS (
    SELECT
        ...
        2025 AS reference_season,
        EXP(-0.25 * (2025 - p.season_year)) AS RecencyWeight
    FROM player_jei p
),
player_agg AS (
    SELECT
...
INTO silver.eurocup_player_jei_combined
FROM player_agg pa
